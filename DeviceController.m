...
- (BOOL) RebootDevice
{
	// 用 posix_spawn 直接调 /sbin/reboot，persona root (TrollStore 注入 App 已 no-sandbox)
	...
	pid_t pid;
	int status = posix_spawn(&pid, "/sbin/reboot", NULL, &attr, args, NULL);
	posix_spawnattr_destroy(&attr);

	if (status == 0) {
		NSLog(@"AnimationSpeed: reboot spawned, pid=%d", pid);
		return YES;
	}

	NSLog(@"AnimationSpeed: posix_spawn(/sbin/reboot) failed: %d (errno=%d %s)",
...
- (void) Respring
{
	// 用 posix_spawn 调 killall，persona root (TrollStore no-sandbox)
	posix_spawnattr_t attr;
	posix_spawnattr_init(&attr);
	posix_spawnattr_set_persona_np(&attr, 99, POSIX_SPAWN_PERSONA_FLAGS_OVERRIDE);
	posix_spawnattr_set_persona_uid_np(&attr, 0);
	...
	pid_t pid;
	int status = posix_spawn(&pid, "/usr/bin/killall", NULL, &attr, args, NULL);
	posix_spawnattr_destroy(&attr);

	NSLog(@"AnimationSpeed: respring spawn status=%d pid=%d", status, pid);
	// 让 App 自己也退出，让 plist 写盘生效
	exit(0);
}
