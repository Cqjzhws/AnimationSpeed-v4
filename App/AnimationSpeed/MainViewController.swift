...
class MainViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    let versionCode = "1.1.0"
    
    private var tableView = UITableView()
    
    // 分组结构：0=速度设置  1=快速档位  2=恢复默认  3=关于
    private let tableTitleList = [
        NSLocalizedString("SpeedSettings", comment: ""),
        NSLocalizedString("QuickPresets", comment: ""),
        nil,
    ...
    private let tableCellList = [
        ...
        [
            NSLocalizedString("PresetSlightly", comment: ""),
            NSLocalizedString("PresetFast", comment: ""),
            NSLocalizedString("PresetFaster", comment: ""),
            NSLocalizedString("PresetInstant", comment: "")
        ],
        [NSLocalizedString("RestoreDefault", comment: "")],
        [NSLocalizedString("Version", comment: ""), "GitHub", NSLocalizedString("Reference", comment: "")]
    ]
    
    // 快速档位对应的 UIAnimationDragCoefficient 数值
    // 参考：12%≈0.893  25%≈0.8  50%≈0.667  100%≈0.5  越大越慢
    private let presetValues: [Double] = [0.8, 0.667, 0.5, 0.001]
    
    // 标记分组编号，防止新增分组需要改多处代码
    private let presetSection = 1
    private let aboutAtSection = 3
    ...
    func tableView(_ tableView: UITableView, titleForFooterInSection section: Int) -> String? {
        if section == presetSection {
            return NSLocalizedString("PresetFooter", comment: "")
        }
        return nil
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        } else if indexPath.section == presetSection { // 快速档位
            let current = AnimationHelper.currentUIAnimationDragCoefficient()
            let preset = presetValues[indexPath.row]
            let isCurrent = abs(current - preset) < 0.0005
            if isCurrent {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 1 { // 应用按钮的点击事件
                ...
                if let cell = tableView.cellForRow(at: IndexPath(row: 0, section: 0)), let textField = cell.contentView.viewWithTag(1) as? UITextField {
                    ...
                    confirmAndApply(newValue: newValue)
                }
            }
        } else if indexPath.section == presetSection { // 点击快速档位
            if !hasRootPermission { // 无root权限不响应操作
                return
            }
            let preset = presetValues[indexPath.row]
            let current = AnimationHelper.currentUIAnimationDragCoefficient()
            if abs(current - preset) < 0.0005 { // 已经是当前档位
                return
            }
            confirmAndApply(newValue: preset)
        } else if indexPath.section == 2 {
            if !hasRootPermission { // 无root权限不响应操作
                return
            }
    ...
    // MARK: - 确认并写入动画速度（手动输入与快速档位共用）
    // 重启/注销按键逻辑与原版保持一致，未做改动。
    private func confirmAndApply(newValue: Double) {
        // 与原版一致：文件已存在则走重启，否则注销即可生效
